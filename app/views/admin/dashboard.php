...
<div class="row" id="info_box">
    <div class="col-12 col-sm-6 col-md-3">
        <div class="info-box"> <span class="info-box-icon text-bg-primary shadow-sm"> <i class="bi bi-gear-fill"></i> </span>
            <div class="info-box-content"> <span class="info-box-text">سفارشات جدید</span> <span class="info-box-number numwc">
                    <?php echo $newOrdersCount; ?>
                    <small>تعداد</small> </span> </div> <!-- /.info-box-content -->
        </div> <!-- /.info-box -->
    </div> <!-- /.col -->
    <div class="col-12 col-sm-6 col-md-3">
        <div class="info-box"> <span class="info-box-icon text-bg-danger shadow-sm"> <i class="bi bi-hand-thumbs-up-fill"></i> </span>
            <div class="info-box-content"> <span class="info-box-text">استعلام جدید</span> <span class="info-box-number numwc"><?php echo $newQuotationsCount; ?></span> </div> <!-- /.info-box-content -->
        </div> <!-- /.info-box -->
    ...
    <div class="col-12 col-sm-6 col-md-3">
        <div class="info-box"> <span class="info-box-icon text-bg-success shadow-sm"> <i class="bi bi-cart-fill"></i> </span>
            <div class="info-box-content"> <span class="info-box-text">طلب از کاربران</span> <span class="info-box-number numwc"><?php echo $userBalance; ?></span> </div> <!-- /.info-box-content -->
        </div> <!-- /.info-box -->
    </div> <!-- /.col -->
    <div class="col-12 col-sm-6 col-md-3">
        <div class="info-box"> <span class="info-box-icon text-bg-warning shadow-sm"> <i class="bi bi-people-fill"></i> </span>
            <div class="info-box-content"> <span class="info-box-text">بدهی به بروکرها</span> <span class="info-box-number numwc"><?php echo $brokerBalance; ?></span> </div> <!-- /.info-box-content -->
        </div> <!-- /.info-box -->
    </div> <!-- /.col -->
</div>
<!-- نمودار -->
...
<div class="row"> <!-- SECTION 2 -->
    <div class="col-md-8"> <!-- اخرین سفارشات -->
        <div class="card card-primary card-outline g-4 mb-4 last_order">
            <div class="card-header">
                ...
                <table class="table table-responsive table-striped">
                    ...
                    <tbody>
                        <?php foreach ($lastOrders as $order) : ?>
                            <tr>
                                <td><span class="dur_date"><?php echo $order['created_at']; ?></span></td>
                                <td><?php echo $order['operator_username']; ?></td>
                                <td><img src="<?php echo $order['company_icon']; ?>" style="width:24px;" alt="<?php echo $order['company_name']; ?>"> <?php echo $order['package_name']; ?></td>
                                <td><?php echo $order['user_name'] . ' ' . $order['user_surname']; ?></td>
                                <td><span class="numwc"><?php echo $order['payment']; ?></span></td>
                                <td><span class="numwc"><?php echo $order['status']; ?></span></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <div class="card-footer">
                <div class="clearfix">
                    <a href="/admin/orders?showModal=true" class="btn btn-sm btn-primary float-start">سفارش جدید</a>
                    <a href="/admin/orders" class="btn btn-sm btn-secondary float-end">همه</a>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="card card-primary card-outline g-4 mb-4 fast_payment">
            <form id="quick-payment-form">
                <div class="card-header">
                    <h3 class="card-title">ثبت سریع پرداخت</h3>
                </div>
                <div class="card-body">
                    <div class="btn-group mb-2 d-flex" role="group" aria-label="Button group with nested dropdown">
                        <input class="btn-check" autocomplete="off" checked="" type="radio" name="payment_type" value="broker" id="payment_broker" required>
                        <label class="btn btn-outline-primary" for="payment_broker">بروکر</label><br>
                        <input class="btn-check" autocomplete="off" checked="" type="radio" name="payment_type" value="user" id="payment_user" required>
                        <label class="btn btn-outline-primary" for="payment_user">کاربر</label>
                        <div class="form-group col-8">
                            <select class="form-select" id="select_id" name="select_id" required>
                                <option value="">ابتدا نوع کاربر را انتخاب کنید</option>
                            </select>
                        </div>
                    </div>
                    <div class="input-group mb-3">
                        <span class="input-group-text">مبلغ پرداختی</span>
                        <input type="number" class="form-control" id="amount" name="amount" required aria-label="مبلغ را وارد کنید">
                        <span class="input-group-text">لیر</span>
                    </div>
                </div>
                <div class="card-footer">
                    <a href="/admin/broker-transactions" class="btn btn-sm btn-secondary float-start">تراکنش بروکرها</a>
                    <a href="/admin/transactions" class="btn btn-sm btn-secondary float-start">تراکنش کاربران</a>
                    <button type="submit" class="btn btn-primary float-end">ثبت پرداخت</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="row"> <!-- SECTION 3 -->
    <div class="col-md-4"> <!-- استعلام -->
        <div class="card card-primary g-4 mb-4 last_quotation">
            <div class="card-header">
            ...
            <div class="card-body">
                <table class="table table-responsive table-striped">
                    ...
                    <tbody>
                        <?php foreach ($lastQuotations as $quotation) : ?>
                            <tr>
                                <td> <a href="/offers/<?php echo $quotation['uid']; ?>" target="new"><?php echo $quotation['id']; ?></a></td>
                                <td class="dur_date"><?php echo $quotation['created_at']; ?></td>
            ...
            <div class="card-footer">
                <div class="clearfix">
                    <a href="/admin/quotations/create" class="btn btn-sm btn-primary float-start">جدید</a>
                    <a href="/admin/quotations" class="btn btn-sm btn-secondary float-end">همه</a>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-4"> <!-- تولدها -->
        <div class="card card-success g-4 mb-4 user_bdate">
    ...
    <div class="col-md-4"> <!-- یادآور تمدید -->
        <div class="card card-danger g-4 mb-4 renew_reminder">
            <div class="card-header"> <!-- card header -->
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        document.getElementById('send-reminder').addEventListener('click', function() {
            ...
            // Create quotation
            fetch('/admin/quotations/createFromOrder', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        function fetchChartData(days) {
            fetch(`/admin/getChartData?days=${days}`)
                .then(response => response.json())
                .then(data => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('input[name="payment_type"]').forEach(function(element) {
            element.addEventListener('change', function() {
                ...
                fetch(`/admin/get${paymentType === 'broker' ? 'Brokers' : 'Users'}`)
                    .then(response => response.json())
                    .then(data => {
        ...
        document.getElementById('quick-payment-form').addEventListener('submit', function(event) {
            ...
            fetch(`/admin/fast/${paymentType === 'broker' ? 'broker-transactions' : 'transactions'}`, {
                    method: 'POST',
                    body: formData
                }).then(response => response.json())
                .then(data => {
